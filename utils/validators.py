...
class Validators:
    ...
    def validar_configuracion_completa(config: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        campos_requeridos = ['contenedor', 'modelos']
        
        for campo in campos_requeridos:
            if campo not in config or not config[campo]:
                return False, f"Falta la configuración: {campo}"
            
            # Validar que los archivos existen
            es_valido, mensaje = Validators.validar_archivo_excel(config[campo])
            if not es_valido:
                return False, f"Archivo de {campo}: {mensaje}"
